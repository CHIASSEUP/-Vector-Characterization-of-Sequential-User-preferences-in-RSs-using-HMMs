...
void ConstructMCsOfUser(User u, MC TheMCs[MaxFeatures], int SequencesOfUser[MaxFeatures][MaxWatchedMovies], int NbFeatures, Movie TheMovies[MaxMovies], int NbMovies, int FrequencyOfGenres[MaxGenres], int NbClustersGenres, FILE ** f){
	...
	for(i=0;(i < u.NbWatchedMovies);i++){
		...
		if(MovieIndex != -1){
			...
			TheMCs[3].Elements[i].Symbol = SymbolOf_Genres(TheMovies[MovieIndex].Genres,TheMovies[MovieIndex].NbGenres,FrequencyOfGenres,NbClustersGenres);
...
// KNN algorithm 
void KNN(int NbNeighbours, int NbVectors, int NbComponents, int Dist, double FinalVect[MaxUsers][MaxFeatures*MaxSymbols], int TheNeighbours[MaxUsers][MaxNeighbours], double NeighbourDistances[MaxUsers][MaxNeighbours]) {
    ...
    for (i = 0; i < NbVectors; i++) {
        ...
        for (j = 0; j < NbVectors; j++) {
            if (i != j) {
				if(Dist == 0){
					distances[j] = DistanceEuclidienne(FinalVect[i], FinalVect[j], NbComponents);
				}else{
					distances[j] = DistanceManhattan(FinalVect[i], FinalVect[j], NbComponents);
				}
            } else {
...
// Computes the F1-measure of the system
double F1_measure(User TheUsers[MaxUsers], int NbUsers, MovieRecom TheMovieRecom[MaxUsers][MaxNeighbours*MaxWatchedMovies], int MaxRecoms, double * std_dev){
	double Intersect, Precision, Recall, F1_user[MaxUsers], F1, epsi = 1e-20;
	int i,j,k,TotalIntersect;
	...
	for (i = 0; i < NbUsers; i++) {
		...
		Precision = (Intersect / MaxRecoms);
		Recall = (Intersect / TheUsers[i].NbWatchedMovies);
		F1_user[i] = (2.0*Precision*Recall)/(Precision + Recall + epsi);
		F1 += F1_user[i];
	}	
	...
	(*std_dev) = 0.0;
	for (i = 0; i < NbUsers; i++) {
		(*std_dev) += (F1_user[i] - F1)*(F1_user[i] - F1);
	}
	(*std_dev) /= NbUsers;
