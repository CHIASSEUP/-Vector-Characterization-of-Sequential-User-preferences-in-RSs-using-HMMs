...
int main(int NbArg, char ** MesArg) {
	...
	NbClusters = 4;
	NbFeatures = atoi(MesArg[5]);
	MaxIterations = atoi(MesArg[6]);
	NbNeighbours = atoi(MesArg[7]);
	MaxRecoms = atoi(MesArg[8]); 
	Tmax = atoi(MesArg[9]);
	if(Tmax > MaxWatchedMovies){
		Tmax = MaxWatchedMovies;
	}
	SaveHMMs = atoi(MesArg[10]);
	...
	for(i=0;(i < NbUsers);i++){
		...
		ConstructMCsOfUser(TheUsers[i],TheMCsOfUser,SequencesOfUser,NbFeatures,TheMovies,NbMovies,FrequencyOfGenres,NbClusters,&f);
